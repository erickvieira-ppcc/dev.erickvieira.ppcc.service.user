...
@EnableEurekaClient
@ComponentScan(basePackages = ["dev.erickvieira.ppcc", "dev.erickvieira.ppcc.service.user"])
open class UserServiceApplication

val logger: Logger = LoggerFactory.getLogger(UserServiceApplication::class.java)

fun main(
    args: Array<String>
) = SpringApplication(UserServiceApplication::class.java).run(*args).environment.let { env ->
    ...
    val contextPath = env.getProperty("server.servlet.context-path").let {
        if (it.isNullOrBlank()) "api/v1/user" else it
    }
